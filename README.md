...
_**Задание 1**_
<details>
	<summary>Используя базу, полученную в лабораторной 2, создать транзакцию, произвести ее откат и фиксацию. Показать, что данные существовали до отката, удалились после отката, снова были добавлены, и затем были успешно зафиксированы. При необходимости используйте точки сохранения и вложенные транзакции.</summary>
<pre><code>USE [Library]
GO
--
DELETE 
FROM [dbo].[Читатели] 
WHERE [Фамилия] = 'Тестов'
--
SELECT 'До транзакции' AS [Состояние], COUNT(*) AS [Количество записей] 
FROM [dbo].[Читатели]
--
BEGIN TRANSACTION ОсновнаяТранзакция
--
    SAVE TRANSACTION До_Отката
--
    INSERT INTO [dbo].[Читатели] ([Фамилия], [Имя], [Отчество], [Задолженность], [Id_группы])
    VALUES ('Тестов', 'Тест', 'Тестович', 100.00, 1)
--
    SELECT 'После добавления в транзакции' AS [Состояние], COUNT(*) AS [Количество записей] 
    FROM [dbo].[Читатели]
--
    ROLLBACK TRANSACTION До_Отката
--
    SELECT 'После отката к точке сохранения' AS [Состояние], COUNT(*) AS [Количество записей] 
    FROM [dbo].[Читатели]
--
    INSERT INTO [dbo].[Читатели] ([Фамилия], [Имя], [Отчество], [Задолженность], [Id_группы])
    VALUES ('Тестов', 'Тест', 'Тестович', 0.00, 1)
--
    SELECT 'После повторного добавления' AS [Состояние], COUNT(*) AS [Количество записей] 
    FROM [dbo].[Читатели]
--
COMMIT TRANSACTION ОсновнаяТранзакция
--
SELECT 'После фиксации транзакции' AS [Состояние], COUNT(*) AS [Количество записей] 
FROM [dbo].[Читатели]
--
SELECT * 
FROM [dbo].[Читатели] 
...
_**Задание 2**_
<details>
	<summary>Подготовить SQL-скрипты для выполнения проверок изолированности транзакций. Ваши скрипты должны работать с одной из таблиц, созданных в лабораторной работе №2.</summary>
<pre><code>INSERT INTO [dbo].[Книги] ([Название], [Цена], [Год_издания], [Число_экземпляров], [Id_издательства])
VALUES ('Тестовая книга', 100.00, '2025-01-01', 10, 1)
</code></pre>
</details>

_**Выполнение работы**_
<ol type="1"> 
	<li>
Запустить клиента и соединиться с базой данных. Открыть второе окно для ввода текста запросов (Ctrl+N в первом окне). 
	</li>
	<li>Установить в обоих сеансах уровень изоляции READ UNCOMMITTED. Выполнить сценарии проверки:
		<ul> 
			<li>
				<details>
					<summary>потерянных изменений,</summary>
				Транзакция 1
<pre><code>SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
BEGIN TRANSACTION
--
SELECT [Число_экземпляров] FROM [dbo].[Книги] WHERE [Название] = 'Тестовая книга'
--
DECLARE @Число_экземпляров INT 
--
SELECT @Число_экземпляров = [Число_экземпляров] FROM [dbo].[Книги] WHERE [Название] = 'Тестовая книга'
--
WAITFOR DELAY '00:00:05'
--
UPDATE [dbo].[Книги] 
SET [Число_экземпляров] = @Число_экземпляров + 1
WHERE [Название] = 'Тестовая книга'
--
COMMIT TRANSACTION
--
SELECT [Число_экземпляров] FROM [dbo].[Книги] WHERE [Название] = 'Тестовая книга'
...
--
UPDATE [dbo].[Книги] 
SET [Число_экземпляров] = @Число_экземпляров + 2
WHERE [Название] = 'Тестовая книга'
--
COMMIT TRANSACTION
--
SELECT [Число_экземпляров] FROM [dbo].[Книги] WHERE [Название] = 'Тестовая книга'
			...
			<li>
				<details>
					<summary>грязного чтения.</summary>
				Транзакция 1
<pre><code>SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
BEGIN TRANSACTION
--
SELECT * FROM [dbo].[Книги] WHERE [Название] = 'Тестовая книга'
--
UPDATE [dbo].[Книги] 
SET [Число_экземпляров] = [Число_экземпляров] + 1
WHERE [Название] = 'Тестовая книга'
...
UPDATE [dbo].[Книги] 
SET [Цена] = 200
WHERE [Название] = 'Тестовая книга' AND @Число_экземпляров > 10
--
COMMIT TRANSACTION
--
SELECT * FROM [dbo].[Книги] WHERE [Название] = 'Тестовая книга'
...
SELECT * FROM [dbo].[Книги] WHERE [Название] = 'Тестовая книга'
			...
			<li>
				<details>
					<summary>неповторяющегося чтения</summary>
				Транзакция 1
<pre><code>SET TRANSACTION ISOLATION LEVEL READ COMMITTED
BEGIN TRANSACTION
--
...
COMMIT TRANSACTION
			...
			<li>
				<details>
					<summary>фантома</summary>
				Транзакция 1
<pre><code>SET TRANSACTION ISOLATION LEVEL REPEATABLE READ
BEGIN TRANSACTION
--
SELECT * FROM [dbo].[Книги] WHERE [Число_экземпляров] > 100
--
WAITFOR DELAY '00:00:05'
--
SELECT * FROM [dbo].[Книги] WHERE [Число_экземпляров] > 100
...
SELECT * FROM [dbo].[Книги] WHERE [Число_экземпляров] > 100
--
INSERT INTO [dbo].[Книги] ([Название], [Цена], [Год_издания], [Число_экземпляров], [Id_издательства])
VALUES ('Тестовая книга2', 100.00, '2025-01-01', 101, 1)
--
COMMIT TRANSACTION
--
SELECT * FROM [dbo].[Книги] WHERE [Число_экземпляров] > 100
	...
	<li>Записать протокол выполнения сценариев.</li>
	<li>Установить в обоих сеансах уровень изоляции SERIALIZABLE. Выполнить сценария проверки 
		<ul> 
			<li>
				<details>
					<summary>фантома</summary>
...
- Грязное чтение. Транзакция Т1 изменяет некоторые данные, но еще не завершается. Транзакция Т2 читает эти же данные (с изменениями, внесенными транзакцией Т1) и принимает на их основе какие-то 3 решения. Транзакция Т1 выполняет откат. В результате решение, принятое транзакцией Т2 основано на неверных данных. 
...
<img width="974" height="427" alt="image" src="https://github.com/user-attachments/assets/940658b2-deae-4b1e-9b3e-3ef2e2d9243d" />

<h3>
  <a href="https://github.com/kirill-155/DateBase/blob/main/Ресурсы/Лабораторная%207.docx" target="_blank">Запросы лабораторной 7 (со скринами)</a>
</h3>
...
4. Найдите рестораны, которые:
   - не относятся к типу кухни `American`,
   - получили оценку «A»,
   - не расположены в районе `Brooklyn`.  
   Документы должны быть отсортированы по полю `cuisine` в порядке убывания.
5. Найдите `restaurant_id`, `name`, `borough` и `cuisine` для тех ресторанов, чьё название начинается с «Wil».
6. Найдите рестораны, которые относятся к району `Bronx` и готовят `American` или `Chinese` блюда.
7. Найдите `restaurant_id`, `name` и `grades` для тех ресторанов, у которых **11 августа 2014 года** (`2014-08-11T00:00:00Z`) была выставлена оценка `A` со счётом `9`.
8. В каждом районе посчитайте количество ресторанов по каждому виду кухни. Результат должен содержать поля: `borough`, `cuisine`, `count`.
9. В районе `Bronx` найдите ресторан с минимальной суммой набранных баллов.
10. Добавьте в коллекцию свой любимый ресторан.
11. В добавленном ресторане укажите информацию о времени его работы (например, поле `opening_hours`).
...
#### Выполните следующие агрегационные запросы (`Aggregate`):
...
3. Найдите первые 10 записей с самой низкой температурой, когда дул ветер с юга, и посчитайте среднюю температуру для этих записей.
4. Подсчитайте количество дней, когда шёл снег. (Снег — это осадки при температуре ниже 0°C.)
5. В течение зимы иногда шёл снег, а иногда дождь. Насколько больше (или меньше) выпало осадков в виде снега?
6. Какова вероятность того, что в ясный день выпадут осадки? (День считается ясным, если ясная погода фиксировалась более чем в 75% измерений за этот день.)
7. Увеличьте температуру на 1 градус при каждом измерении в нечётный день во время зимы. На сколько градусов изменилась средняя температура?


<h3>
  <a href="https://github.com/kirill-155/DateBase/blob/main/Ресурсы/Лабораторная%208.docx" target="_blank">Запросы лабораторной 8 (со скринами)</a>
</h3>
